function Invoke-CpridutilClish {
    <#
    .SYNOPSIS
        Executes a clish command on a Check Point gateway via cprid_util.

    .DESCRIPTION
        Wrapper around Invoke-Cpridutil that executes a clish command on a remote gateway.
        Automatically parses the output: attempts JSON conversion first, then regex matching
        if a pattern is provided, otherwise returns the output split by lines. Throws an error
        if the task fails or SIC communication is broken.
    ...
    .PARAMETER Script
        The clish command to execute on the gateway.
    ...
    .OUTPUTS
        [PSCustomObject] JSON-parsed output, [Hashtable] regex matches, or [String[]] output lines.

    .EXAMPLE
        Invoke-CpridutilClish -Firewall "GW01" -Script "show route"

    .EXAMPLE
        Invoke-CpridutilClish -Firewall "GW01" -Script "show configuration static-route" -Regex "nexthop gateway address (?<defgw>.+) on"
